...
public class XAnnotationAssocManys extends AnnotationAssocManys {
    ...
    for (DeployBeanProperty prop : descriptor.propertiesAll()) {
      if (prop instanceof DeployBeanPropertyAssocMany<?>) {
        if (prop.getField() == null) {
          ...
          parseField(field, (DeployBeanPropertyAssocMany<?>) prop);
        }
      }
    }
  }

  private void parseField(XField field, DeployBeanPropertyAssocMany<?> prop) {
    this.field = field;
    read(prop);
    this.field = null;
  }
